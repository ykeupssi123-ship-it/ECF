#!/bin/bash
# ECFREMP2 - ECF_ILL2_RUN_EMPLOYES - Illustration : cree une
...
# la demo. IN_COND=RH_ACTIVE (voir ILL1_EMPLOYES.sh pour le detail
...
echo "[ILL2_EMPLOYES] Creation de l'equipe COUL..."
_odoo_shell_exec "
company = env['res.company'].search([('name', '=', 'COUL')], limit=1)
assert company, 'societe COUL introuvable - jouer ILL2_SOCIETE d abord'
...
echo "[ILL2_EMPLOYES] Verification reelle en base..."
...
if [ "${NB:-0}" -lt 4 ]; then
  echo "[ILL2_EMPLOYES] ERREUR : seulement ${NB:-0}/4 employes trouves en base." >&2
  exit 1
fi

echo "[ILL2_EMPLOYES] OK (${NB} employes COUL en base)."
exit 0
