#!/bin/bash
# ECFRREC1 - ECF_ILL1_RUN_RECRUTEMENT - Illustration :
# cree une offre d'emploi reelle + un candidat pour CLIM AUTO -
# approfondit le volet RH demande par le client.
# IN_COND=RECRUTEMENT_ACTIVE|ILL1_SOCIETE_OK.
set -uo pipefail
source "$VARS_FILE"
PROJECT_ROOT="$(dirname "$VARS_FILE")"
source "$PROJECT_ROOT/lib/commun.sh"

echo "[ILL1_RECRUTEMENT] Creation d'une offre + candidat CLIM AUTO..."
_odoo_shell_exec "
...
echo "[ILL1_RECRUTEMENT] Verification reelle en base..."
...
if [ "${NB:-0}" -lt 1 ]; then
  echo "[ILL1_RECRUTEMENT] ERREUR : candidat introuvable en base." >&2
  exit 1
fi

echo "[ILL1_RECRUTEMENT] OK."
exit 0
