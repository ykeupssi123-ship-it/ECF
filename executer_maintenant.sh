...
if [ $JOB_EXIT -eq 0 ]; then
  ...
  echo "$(date -Iseconds),$JOB_ID,$C_JOB_NAME,EXEC_MAINTENANT_OK,$JOB_LOG,$JOB_DURATION_SEC" >> "$HISTORY_LEDGER"
  echo "$JOB_ID -> EXEC_MAINTENANT_OK ($C_OUT_COND)."
  exit 0
else
  echo "$(date -Iseconds),$JOB_ID,$C_JOB_NAME,EXEC_MAINTENANT_ECHEC,$JOB_LOG,$JOB_DURATION_SEC" >> "$HISTORY_LEDGER"
