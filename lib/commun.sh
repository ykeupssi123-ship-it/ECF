...
# SUPPRESSION/RESTAURATION (DELETE/UNDELETE), ajoutees le 2026-09-01 -
# equivalent fonctionnel du "Delete"/"Undelete" Control-M : retire un job
# du plan d'execution courant (jamais liste comme "en attente", jamais
# tente par l'orchestrateur), distinct du gel (HELD reste visible comme
# "en attente, gele" - DELETED disparait du radar operationnel courant,
# comme retire de l'AJP du jour). Voir bin/delete_job.sh/bin/undelete_job.sh.
job_deleted(){ [ -f "${STATE_DIR}/DELETED/$1.deleted" ]; }

# CONFIRMATION MANUELLE PREALABLE (CONFIRM), ajoutee le 2026-09-01 -
# equivalent fonctionnel du "Confirm" Control-M : un job marque ainsi
# (a l'avance, par l'operateur) reste bloque tant qu'un humain n'a pas
# explicitement approuve son execution, meme si toutes ses dependances
# sont satisfaites - distinct de HELD (qui est une decision d'exploitation
# generale) : CONFIRM_REQUIS cible un besoin d'approbation ponctuel avant
# UNE execution precise (ex: envoi reel d'une facture par email a un
# client). Voir bin/confirm_job.sh.
job_needs_confirm(){ [ -f "${STATE_DIR}/CONFIRM_REQUIS/$1.confirm" ]; }
...
# ACTIVATION/DESACTIVATION REELLE D'UN MODULE ODOO, ajoutees le
# 2026-09-01 (foret de jobs Tier 1 - un job par module reel du
# catalogue, voir jobs/ODOO_MODULE_*_<module>.sh generes a partir de
# ce point unique).
#
# Choix technique deliberement DIFFERENT d'ODOO_014_CREATE_DATABASE.sh
# (qui utilise "odoo-bin -i ... --stop-after-init", reserve a
# l'initialisation d'une base neuve, service arrete) : ici, le service
# odoo tourne deja EN PRODUCTION potentielle (des utilisateurs peuvent
# etre connectes) - on ne peut pas se permettre d'arreter/redemarrer le
# service a chaque activation/desactivation de module. "odoo-bin shell"
# execute le MEME code Python que le bouton "Activer"/"Desinstaller" de
# l'interface web (ir.module.module.button_immediate_install /
# button_immediate_uninstall) contre l'instance DEJA EN COURS
# d'execution - jamais un raccourci ou une simulation, exactement ce
# qu'un clic reel declenche.
#
# Verification reelle : l'etat en base (ir_module_module.state) est
# relu apres coup, jamais suppose a partir du seul code de sortie du
# shell Odoo (meme discipline que partout ailleurs dans ce projet).
#
# Usage : odoo_module_activate <nom_technique> (ex: "crm", "point_of_sale")
#         odoo_module_deactivate <nom_technique>
_odoo_shell_exec(){
  local python_code="$1"
  sudo -u "${ODOO_USER}" bash -c "source '${ODOO_HOME}/venv/bin/activate' && cd '${ODOO_HOME}/odoo-src' && timeout 300 python3 odoo-bin shell -c '${ODOO_CONF}' -d '${PG_ODOO_DB_NAME}' --no-http --shell-interface=python <<'PYEOF'
${python_code}
PYEOF"
}
...
_odoo_module_state(){
  local module="$1"
  local pw
  pw="$(read_or_generate_secret "$PG_ODOO_DB_PASSWORD_FILE" non)" || return 1
  PGPASSWORD="$pw" psql -h 127.0.0.1 -U "${PG_ODOO_DB_USER}" -d "${PG_ODOO_DB_NAME}" -tAc \
    "SELECT state FROM ir_module_module WHERE name='${module}';"
}

# CORRIGE le 2026-09-01 (echec reel : MOD_POS_RESTO_DEACTIVATE, voir
# docs/JOURNAL_TECHNIQUE.md) : "button_immediate_install/uninstall" pose
# un verrou reel ("SELECT FROM ir_cron FOR UPDATE") pour interdire toute
# operation de module pendant qu'une tache planifiee (cron) interne
# d'Odoo est en cours - protection deliberee d'Odoo lui-meme contre une
# corruption du registre, pas un bug. En enchainant 34 modules
# activation/desactivation rapidement, la fenetre de collision avec le
# worker cron interne (actif meme hors production, ex: file d'attente
# mail) devient reelle. Meme famille de lecon que le clone Git de
# l'incident 5 (ODOO_009) : contention transitoire -> reprise avec
# nouvelle tentative, jamais une "solution" qui change la logique
# metier.
#
# CORRIGE UNE 2e FOIS le 2026-09-01 (echec reel : MOD_ELEARNING_ACTIVATE,
# meme verrou ir_cron, la 1ere version de ce correctif n'a JAMAIS
# retente) : "odoo-bin shell" est une REPL - une exception Python non
# rattrapee a l'interieur y est affichee (traceback complet, visible
# dans le log) mais n'est PAS propagee comme code de sortie du process,
# qui rend 0 quand meme. Se fier au code de sortie de _odoo_shell_exec
# pour decider s'il faut retenter etait donc totalement inoperant -
# verifie en reel : premiere tentative "reussie" (code 0) alors que
# l'operation avait echoue, aucune 2e tentative declenchee. Seule
# verification fiable : relire l'etat REEL en base (ir_module_module)
# apres chaque tentative, jamais le code de sortie d'une REPL.
odoo_module_activate(){
  local module="$1"
  local etat
  etat="$(_odoo_module_state "$module")"
  if [ "$etat" = "installed" ]; then
    echo "[odoo_module_activate] ${module} deja actif (state=installed), rien a faire."
    return 0
  fi
  echo "[odoo_module_activate] Activation reelle de '${module}' (button_immediate_install, instance en cours d'execution)..."
  local essai
  for essai in 1 2 3; do
    _odoo_shell_exec "
mod = env['ir.module.module'].search([('name', '=', '${module}')], limit=1)
assert mod, 'module ${module} introuvable dans le registre'
if mod.state != 'installed':
    mod.button_immediate_install()
env.cr.commit()
print('RESULTAT:', mod.state)
"
    etat="$(_odoo_module_state "$module")"
    [ "$etat" = "installed" ] && break
    echo "[odoo_module_activate] ${module} : essai ${essai}/3 sans effet (state='${etat}', verrou ir_cron probable), nouvelle tentative dans 10s..." >&2
    sleep 10
  done
  if [ "$etat" != "installed" ]; then
    echo "[odoo_module_activate] ERREUR : ${module} state='${etat}' apres 3 tentatives (attendu: installed)." >&2
    return 1
  fi
  echo "[odoo_module_activate] OK (${module} : installed, verifie en base)."
}

odoo_module_deactivate(){
  local module="$1"
  local etat
  etat="$(_odoo_module_state "$module")"
  if [ "$etat" != "installed" ]; then
    echo "[odoo_module_deactivate] ${module} deja inactif (state=${etat:-absent}), rien a faire."
    return 0
  fi
  echo "[odoo_module_deactivate] Desactivation reelle de '${module}' (button_immediate_uninstall, instance en cours d'execution)..."
  local essai
  for essai in 1 2 3; do
    _odoo_shell_exec "
mod = env['ir.module.module'].search([('name', '=', '${module}')], limit=1)
...
"
    etat="$(_odoo_module_state "$module")"
    [ "$etat" != "installed" ] && break
    echo "[odoo_module_deactivate] ${module} : essai ${essai}/3 sans effet (verrou ir_cron probable), nouvelle tentative dans 10s..." >&2
    sleep 10
  done
  if [ "$etat" = "installed" ]; then
    echo "[odoo_module_deactivate] ERREUR : ${module} encore 'installed' apres 3 tentatives." >&2
    return 1
  fi
  echo "[odoo_module_deactivate] OK (${module} : ${etat}, verifie en base)."
}
