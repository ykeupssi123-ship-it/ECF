...
#
# A la difference de ecf-orchestrateur.service (Type=oneshot, declenche
...
# d'un niveau (vars.conf y reste), BIN_DIR pointe separement vers bin/
# (nouvel emplacement de tableau_de_bord.py).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BIN_DIR="$SCRIPT_DIR/bin"
export VARS_FILE="$SCRIPT_DIR/vars.conf"
source "$VARS_FILE"
DASHBOARD_PORT="${DASHBOARD_PORT:-8088}"
UNIT_PATH="/etc/systemd/system/ecf-tableau-de-bord.service"
...
[Unit]
Description=ERP_CRM_FACTORY - Tableau de bord visuel (toile de dependances, lecture seule)
...
systemctl enable --now ecf-tableau-de-bord
...
sleep 2
if systemctl is-active --quiet ecf-tableau-de-bord; then
  echo "[installer_service_tableau_de_bord] Service actif (systemctl is-active confirme)."
else
  echo "[installer_service_tableau_de_bord] ERREUR : le service ne semble pas actif. Voir : journalctl -u ecf-tableau-de-bord -n 40" >&2
  exit 1
fi
...
if command -v curl &>/dev/null; then
  if curl -s -o /dev/null -w "" --max-time 5 "http://127.0.0.1:${DASHBOARD_PORT}/"; then
    ...
    echo "[installer_service_tableau_de_bord] AVERTISSEMENT : pas de reponse HTTP locale sur le port ${DASHBOARD_PORT} - voir journalctl -u ecf-tableau-de-bord." >&2
  fi
fi
...
echo "Consulter les logs        : journalctl -u ecf-tableau-de-bord -f"
exit 0
