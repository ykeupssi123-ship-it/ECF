...
for pass in $(seq 1 $MAX_PASSES); do
  ...
  while IFS=',' read -r JOB_ID JOB_NAME JOB_ROLE COMPONENT SCRIPT_FILE DESC IN_COND OUT_COND; do
    ...
    # (job_deleted) et supprimer_job.sh/restaurer_job.sh. Un job supprime
    # est retire du plan courant, jamais tente, jamais liste comme "en
    # attente" par statut_live.sh.
    if job_deleted "$JOB_ID"; then
      log "$JOB_ID -> SUPPRIME (DELETE), saute. Restaurer avec ./restaurer_job.sh $JOB_ID"
      continue
    fi

    # CONFIRMATION PREALABLE REQUISE (CONFIRM), ajoutee le 2026-09-01 -
    # voir lib/commun.sh (job_needs_confirm) et confirmer_job.sh. Meme
    # traitement que HELD (ni OK ni ECHEC, saute silencieusement tant que
    # non confirme).
    if job_needs_confirm "$JOB_ID"; then
      log "$JOB_ID -> CONFIRMATION REQUISE, saute. Approuver avec ./confirmer_job.sh $JOB_ID"
      continue
    fi
