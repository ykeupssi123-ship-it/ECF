...
if [ $JOB_EXIT -eq 0 ]; then
  ...
  echo "$(date -Iseconds),$JOB_ID,$C_JOB_NAME,REJOUE_OK,$JOB_LOG,$JOB_DURATION_SEC,$PATH_TOUCHED" >> "$HISTORY_LEDGER"
  echo "$JOB_ID -> REJOUE_OK ($C_OUT_COND)."
  exit 0
else
  echo "$(date -Iseconds),$JOB_ID,$C_JOB_NAME,REJOUE_ECHEC,$JOB_LOG,$JOB_DURATION_SEC,$PATH_TOUCHED" >> "$HISTORY_LEDGER"
